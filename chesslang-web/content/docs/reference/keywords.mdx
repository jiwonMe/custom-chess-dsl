...
> ⚠️ **핵심:** 여러 조건은 **OR로 결합**됩니다. 하나라도 만족하면 게임이 종료됩니다.
...
| 키워드 | 설명 | 예시 |
|--------|------|------|
| `effect` | 보드 효과 정의 | `effect trap { }` |
| `blocks` | 이동 차단 대상 | `blocks: enemy` |
| `visual` | 시각적 표시 | `visual: "red"` |

**blocks 값:**
- `none`: 차단 없음 (기본값)
- `enemy`: 효과 소유자의 적 기물 차단
- `friend`: 효과 소유자의 아군 기물 차단
- `all`: 모든 기물 차단
...
| `optional` | 선택적 트리거 | `optional: true` |
| `description` | 선택 다이얼로그 메시지 | `description: "실행할까요?"` |
...
| `mark` | 효과 적용 | `mark origin with trap` |
| `move` | 기물 이동 | `move piece to target` |
| `win` | 승자 선언 | `win White` |
| `lose` | 패자 선언 | `lose Black` |
| `draw` | 무승부 선언 | `draw` |
| `cancel` | 현재 이벤트 취소 | `cancel` |
| `for` | 루프 실행 | `for p in pieces: remove p` |
| `if` | 조건부 실행 | `if x > 0: set y = 1` |
...
on, when, do, where, optional, description, blocks, visual,
if, else, for, while, break, continue, return,
function, let, const, script, true, false, null, and, or, not,
set, create, remove, transform, mark, win, lose, cancel,
